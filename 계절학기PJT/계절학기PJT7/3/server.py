from socket import *                         # 혼자서 처음부터 끝까지 코드를 짜기는 힘들어서,
# 블로그에서 여러가지 코드를 보고 이해하는 방향으로 과제 진행했습니다.
import threading
...
def receive(client_socket, addr, user):
    while 1:
        data = client_socket.recv(1024)      # 클라이언트 소켓에서 데이터를 받아 오기
        string = data.decode()               # 받아온 데이터는 비트로 인코딩 되있기 때문에 디코딩
...
while True:
    ...
    # 처음 클라이언트 소켓이 정의되고 난 후 처음 받는 데이터
