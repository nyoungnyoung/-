...
# 로컬에서 진행하기 때문에 로컬 주소와 지정된 포트번호로 소켓 생성
...
def handle_receive(client_socket, user):       # 서버로 부터 데이터를 받는 함수
    while 1:
        try:
            # 서버로 부터 데이터가 온다면 데이터를 프린트하고 오지 않는 다면
...
receive_thread = threading.Thread(
    target=handle_receive, args=(client_socket, user,))
...
client_socket.sendall(user.encode())  # 클라이언트 소켓이 정의 된 후 보내는 첫 데이터 유저의 이름을 보냄
